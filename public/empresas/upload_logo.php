...
function redirect_with($params){
	$base = '/sistema_escalacao/public/empresas/profile.php';
...
$id = isset($_POST['id']) ? (int)$_POST['id'] : 0;
if ($id <= 0) {
	redirect_with(['err' => 'Empresa inválida']);
}
...
$stmt = $mysqli->prepare('UPDATE empresas SET foto_path = ? WHERE id = ?');
$stmt->bind_param('si', $relativePath, $id);
$stmt->execute();

redirect_with(['msg' => 'Logo atualizada com sucesso']);
